...
export default function Navbar() {
    ...
    const [profileComplete, setProfileComplete] = useState(null); // null = cargando, true o false luego
    ...
    // Mientras cargamos perfil, no mostramos nada para evitar parpadeo
    if (profileComplete === null) {
        return null; 
        // o puedes devolver un spinner o loading si quieres, por ejemplo:
    ...
    return (
        <div style={{ backgroundColor: '#032c3d' }} className="min-h-screen">
            ...
            <div className="flex flex-col items-center text-white mt-10 space-y-4">
                <p className="text-xl font-semibold text-center">¡Quiero aparecer en el apartado de Live's!</p>
                <button
                    onClick={() => router.push('/fetustudios/fetugames2/flive')}
                    className="bg-green-600 hover:bg-green-700 px-6 py-3 rounded text-white font-semibold"
                >
                    Solicita aparecer en Live's
                </button>
            </div>
